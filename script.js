...
// Новые переменные для звука и паузы
...
// Функция, которую вызовет SDK после загрузки данных
window.onYandexDataLoaded = function(data) {
    if (data && typeof data === 'object') {
        ...
        console.log('📦 Нет данных в Яндекс.Играх, загружаем локальные');
        loadSave();
    }
    
    placedCount = 0;
    // Загружаем уровень после получения данных
    loadLevel(currentLevel);
    // Уведомляем, что игра готова (пункт 1.19.2)
    if (window.notifyGameReady) {
...
function loadSave() {
    try {
        ...
        // Загружаем сохранённый прогресс, но уровень может быть не 0, если игра сохраняла
        if (s.level !== undefined) currentLevel = Math.min(s.level, LEVELS.length - 1);
        if (s.score !== undefined) score = s.score;
        if (s.achievements) unlockedAchievements = new Set(s.achievements);
        // dailyDate не используем
    } catch(e) {}
}
...
function saveProgress() {
    try {
        ...
        localStorage.setItem('garageGame', JSON.stringify(data));
        saveToYandex(); // Сохраняем и в Яндекс
    } catch(e) {}
}
...
// Функции для управления звуком (вызываются извне)
window.pauseAllSounds = function() {
    if (isSoundMuted) return; // Если звук выключен, не трогаем
...
window.resumeAllSounds = function() {
    if (isSoundMuted) return; // Если звук выключен, не возобновляем
    Object.values(sounds).forEach(s => {
...
function playSound(name) {
    if (isSoundMuted) return; // Не играем, если звук выключен
    try {
        const s = sounds[name];
        if (s) { 
            s.currentTime = 0; 
...
function toggleSound() {
    ...
    if (isSoundMuted) {
        ...
        Object.values(sounds).forEach(s => {
            s.pause();
            s.dataset.wasPlaying = ''; // Сбрасываем флаг
        });
    } else {
        // Включаем фоновый, если он должен играть
        // (фон включается по клику, так что тут просто снимаем mute)
    }
}

// Включаем фоновый звук при первом клике (если звук не выключен)
...
function togglePause() {
    ...
    if (isPaused) {
        ...
        // TODO: Можно добавить затемнение или меню паузы
    } else {
        if (LEVELS[currentLevel].timeLimit) {
            startTimer(timeLeft); // Возобновляем таймер с того же места
        }
    }
}
...
function openLeaderboard() {
    if (isPaused) return; // Не открываем во время паузы
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Загрузка данных уже запущена через onYandexDataLoaded из index.html
    // Если SDK не загрузится, нужно загрузить локальные данные
    // Добавим таймаут на случай, если SDK не ответит
    setTimeout(() => {
        if (typeof window.onYandexDataLoaded === 'function' && !window.gameIsReady) {
            console.log('⌛ Таймаут SDK, загружаем локально');
            window.onYandexDataLoaded(null);
        }
    }, 2000);
});